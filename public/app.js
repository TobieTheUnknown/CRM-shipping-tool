...
async function showAddColisModal() {
    ...
    displayColisProduitsSelection();
    updateProduitSelect();
    // Réinitialiser la référence en mode auto
    const refInput = document.getElementById('colisReference');
    if (refInput) {
        refInput.value = '';
        refInput.setAttribute('data-auto', 'true');
    }
    // Réinitialiser les dimensions en mode auto
    const dimInput = document.getElementById('colisDimensions');
    if (dimInput) {
        dimInput.value = '';
        dimInput.setAttribute('data-auto', 'true');
    }
...
async function editColis(id) {
    ...
    // Déterminer si la référence est manuelle ou auto
    // Si la référence existante ne correspond pas à celle qui serait générée automatiquement, c'est qu'elle est manuelle
    const refInput = document.getElementById('colisReference');
    const existingRef = c.reference || '';
    const autoRef = colisProduitsSelection
        .map(p => p.ref)
        .filter(ref => ref && ref.trim() !== '')
        .join('+');

    if (existingRef && existingRef !== autoRef) {
        // Référence manuelle
        refInput.setAttribute('data-auto', 'false');
    } else {
        // Référence auto (ou vide)
        refInput.setAttribute('data-auto', 'true');
        updateColisReference();
    }

    // Déterminer si les dimensions sont manuelles ou auto
    const dimInput = document.getElementById('colisDimensions');
    const existingDim = c.dimensions || '';

    // Calculer ce que seraient les dimensions auto
    let autoDim = '';
    ...
    if (maxDimensionId) {
        ...
        if (selectedDim) {
            autoDim = `${selectedDim.longueur}x${selectedDim.largeur}x${selectedDim.hauteur}cm`;
        }
    }

    if (existingDim && existingDim !== autoDim) {
        // Dimensions manuelles
        dimInput.setAttribute('data-auto', 'false');
    } else {
        // Dimensions auto (ou vide)
        dimInput.setAttribute('data-auto', 'true');
        calculatePoidsAndDimension();
    }
...
// Marquer la référence comme saisie manuellement
function markReferenceAsManual() {
    const refInput = document.getElementById('colisReference');
    if (refInput) {
        refInput.setAttribute('data-auto', 'false');
    }
...
// Régénérer automatiquement la référence depuis les produits
function autoGenerateReference() {
    const refInput = document.getElementById('colisReference');
    if (!refInput) return;

    // Marquer comme auto pour permettre les futures mises à jour
    refInput.setAttribute('data-auto', 'true');
    ...
    // Feedback visuel
    refInput.style.background = 'rgba(0, 217, 255, 0.1)';
    setTimeout(() => {
...
function updateColisReference() {
    ...
    // Si la référence est saisie manuellement, ne pas l'écraser
    const isAuto = refInput.getAttribute('data-auto') === 'true';
    if (!isAuto) return;

    if (colisProduitsSelection.length === 0) {
        refInput.value = '';
        refInput.placeholder = 'Saisie manuelle ou auto...';
        return;
    }
...
// Marquer les dimensions comme saisies manuellement
function markDimensionsAsManual() {
    const dimInput = document.getElementById('colisDimensions');
    if (dimInput) {
        dimInput.setAttribute('data-auto', 'false');
    }
...
// Régénérer automatiquement les dimensions depuis les produits
function autoGenerateDimensions() {
    const dimInput = document.getElementById('colisDimensions');
    if (!dimInput) return;
    ...
    // Feedback visuel
    dimInput.style.background = 'rgba(0, 217, 255, 0.1)';
    setTimeout(() => {
        dimInput.style.background = '';
    }, 500);
}
...
function calculatePoidsAndDimension() {
    ...
    // Si les dimensions sont saisies manuellement, ne mettre à jour que le poids
    const isAuto = dimInput && dimInput.getAttribute('data-auto') === 'true';
    ...
    if (isAuto) {
        ...
        if (maxDimensionId) {
            const selectedDim = dimensions.find(d => d.id === maxDimensionId);
            if (selectedDim) {
                dimInput.value = `${selectedDim.longueur}x${selectedDim.largeur}x${selectedDim.hauteur}cm`;
            }
        }
    }

    // Calculer et mettre à jour le poids (toujours)
    updatePoidsTotal();
}
